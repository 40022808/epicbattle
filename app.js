...
function 失败() {
    const 失败 = document.querySelector(".失败")
    失败.style.display = "block"
    热血战斗.pause()
}
...
const 继续游戏_button = document.querySelector("#继续游戏_button")


继续游戏_button.addEventListener('click',()=>{
    继续游戏()
})
...
function 继续游戏() {
    const 胜利 = document.querySelector(".胜利")
    胜利.style.display = "none"
    热血战斗.play()
    boss.style.height = 0 + 'px'
    me.style.height = 0 + 'px'
    HP = 100
    boss_HP = 100
}